...
function App() {
  ...
  const appear=()=>{
    setLarge(!large)
    document.getElementById('appear').style.color='blue';
    setTimeout(document.getElementById('appear').style.display='',1000)
  }
  return (
    <div className='flex  justify-center bg-gray-700 h-screen w-screen  pt-10' >
      ...
      <div id='drop2' className={`${large?'h-[90vh]':'h-[30vh]'} w-96 bg-slate-900 duration-500 flex p-5 rounded-xl items-center flex-col justify-between`}>
        ...
        <div id='appear' className='origin-top delay-300'><div className='overflow-scroll flex flex-col '><p className={`${large?'':'hidden'} text-green-300  text-3xl pt-[2rem] pl-[20rem] origin-right cursor-pointer duration-500 mt-[1rem] pb-3`}>+</p>
